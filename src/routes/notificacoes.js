...
router.post('/', autorizar('ferias', 'escrita'), async (req, res) => {
  try {
    ...
    const tipoLabel = tipo === 'atestado' ? '🏥 Atestado' : '🏖️ Férias';
    const periodo = fimDate ? `${iniDate.toLocaleDateString('pt-BR')} até ${fimDate.toLocaleDateString('pt-BR')}` : `a partir de ${iniDate.toLocaleDateString('pt-BR')}`;
    await criarNotificacao(
      `${tipoLabel} registrado — ${ferias.motorista.nome}`,
      `${ferias.motorista.nome} está de ${tipo === 'atestado' ? 'atestado' : 'férias'} — ${periodo}${observacao ? ` — ${observacao}` : ''}`,
      tipo || 'ferias'
    );
...
router.post('/afastamentos', autorizar('ferias', 'escrita'), async (req, res) => {
  try {
    ...
    await criarNotificacao(
      `⚠️ Afastamento registrado — ${item.motorista.nome}`,
...
router.post('/abandonos', autorizar('ferias', 'escrita'), async (req, res) => {
  try {
    ...
    await criarNotificacao(
      `🚪 Abandono registrado — ${item.motorista.nome}`,
